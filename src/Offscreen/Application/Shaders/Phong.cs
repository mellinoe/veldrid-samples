...
namespace Offscreen.Shaders
{
    public class Phong
    {
        ...
        public Vector4 FS(FSIn input)
        {
            ...
            float diff = Clamp(Vector3.Dot(input.Normal, input.LightVec), 0f, 100000);
            Vector4 IDiffuse = new Vector4(diff);
            float specular = 0.75f;
            Vector4 ISpecular = new Vector4(0.0f);
            if (Vector3.Dot(input.EyePos, input.Normal) < 0.0)
            {
                ISpecular = (new Vector4(0.5f, 0.5f, 0.5f, 1.0f) * Pow(Clamp(Vector3.Dot(Reflected, Eye), 0.0f, 100000), 16.0f)) * specular;
            }

...
namespace PBR
{
    public class PBRApplication : SampleApplication
    {
        ...
        private ResourceSet _pbrResourceSet;
        ...
        private static readonly uint EnvironmentCubeMapMipLevels =
            ComputeMipLevels(EnvironmentCubeMapSize, EnvironmentCubeMapSize);
        ...
        public void SetLight(uint lightIndex, ref AnalyticalLight light)
        {
            if (lightIndex >= NumLights)
                throw new ArgumentOutOfRangeException(
                    $"{nameof(SetLight)} {nameof(lightIndex)} must be less than {NumLights}");
        ...
        private (Texture SpecularEnvironmentCubemap, Texture IrradianceEnvironmentCubemap, Texture SpecularBRDF_LUT)
            LoadAndPreprocessEnvironmentMap(ResourceFactory factory, Sampler textureSampler)
        {
            var environmentTex = LoadEmbeddedAsset<ProcessedTexture>("environment.binary");

            using (var unfilteredEnvCubeMap = factory.CreateTexture(TextureDescription.Texture2D(EnvironmentCubeMapSize,
                EnvironmentCubeMapSize, EnvironmentCubeMapMipLevels, 1, PixelFormat.R16_G16_B16_A16_Float,
                TextureUsage.Sampled | TextureUsage.Cubemap | TextureUsage.GenerateMipmaps)))
            {
                using (var environmentHdrTexture =
                    environmentTex.CreateDeviceTexture(GraphicsDevice, factory, TextureUsage.Sampled))
                {
                    {
                        // convert hdr to cubemap
                        using (var temporaryUnfilteredEnvCubeMap = factory.CreateTexture(
                            TextureDescription.Texture2D(EnvironmentCubeMapSize, EnvironmentCubeMapSize, 1, 6,
                                PixelFormat.R16_G16_B16_A16_Float,
                                TextureUsage.Storage | TextureUsage.Sampled | TextureUsage.RenderTarget)))
                        {
                            ...
                            var equirect2CubeUniformLayoutDescription = new ResourceLayoutDescription(
                                new ResourceLayoutElementDescription("outputTexture", ResourceKind.TextureReadWrite,
                                    ShaderStages.Compute),
                                new ResourceLayoutElementDescription("inputTexture", ResourceKind.TextureReadOnly,
                                    ShaderStages.Compute),
                                new ResourceLayoutElementDescription("textureSampler", ResourceKind.Sampler,
                                    ShaderStages.Compute)
                            );

                            using (var shader = factory.CreateFromSpirv(new ShaderDescription(ShaderStages.Compute,
                                equirect2CubeShaderSrc, "main", Debug)))
                            using (var equirect2CubeUniformLayout =
                                factory.CreateResourceLayout(ref equirect2CubeUniformLayoutDescription))
                            using (var equirect2CubeResourceSet = factory.CreateResourceSet(
                    ...
                    var specularEnvironmentMapTexture = factory.CreateTexture(
                        TextureDescription.Texture2D(EnvironmentCubeMapSize, EnvironmentCubeMapSize,
                            EnvironmentCubeMapMipLevels, 1, PixelFormat.R16_G16_B16_A16_Float,
                            TextureUsage.Sampled | TextureUsage.Cubemap));
                    {
                        // compute pre-filtered specular environment map
                        using (var temporarySpecularEnvironmentMapTexture = factory.CreateTexture(
                            TextureDescription.Texture2D(EnvironmentCubeMapSize, EnvironmentCubeMapSize,
                                EnvironmentCubeMapMipLevels, 6, PixelFormat.R16_G16_B16_A16_Float,
                                TextureUsage.Storage | TextureUsage.Sampled | TextureUsage.RenderTarget)))
                        {
                            ...
                            var spmapUniformLayoutDescription = new ResourceLayoutDescription(
                                ...
                                new ResourceLayoutElementDescription("inputTexture", ResourceKind.TextureReadOnly,
                                    ShaderStages.Compute),
                                new ResourceLayoutElementDescription("FakePushConstants", ResourceKind.UniformBuffer,
                                    ShaderStages.Compute),
                                new ResourceLayoutElementDescription("textureSampler", ResourceKind.Sampler,
                                    ShaderStages.Compute)
                            );
                            ...
                            using (var shader = factory.CreateFromSpirv(new ShaderDescription(ShaderStages.Compute,
                                spmapShaderSrc, "main", Debug)))
                            using (var spmapUniformLayout =
                                factory.CreateResourceLayout(ref spmapUniformLayoutDescription))
                            ...
                            using (var commandList = factory.CreateCommandList())
                            {
                                ...
                                for (uint level = 1, size = EnvironmentCubeMapSize / 2;
                                ...
                                commandList.Begin();
                                commandList.CopyTexture(temporarySpecularEnvironmentMapTexture,
                                    specularEnvironmentMapTexture);
                                commandList.End();
                                GraphicsDevice.SubmitCommands(commandList);
                                GraphicsDevice.WaitForIdle();
                            }
                    ...
                    var irradianceEnvironmentMapTexture = factory.CreateTexture(
                        TextureDescription.Texture2D(kIrradianceMapSize, kIrradianceMapSize, 1, 1,
                            PixelFormat.R16_G16_B16_A16_Float, TextureUsage.Sampled | TextureUsage.Cubemap));
                    {
                        using (var temporaryIrradianceEnvironmentMapTexture = factory.CreateTexture(
                            TextureDescription.Texture2D(kIrradianceMapSize, kIrradianceMapSize, 1, 6,
                                PixelFormat.R16_G16_B16_A16_Float,
                                TextureUsage.Storage | TextureUsage.Sampled | TextureUsage.RenderTarget)))
                        {
                            ...
                            using (var shader = factory.CreateFromSpirv(new ShaderDescription(ShaderStages.Compute,
                                irradianceShaderSrc, "main", Debug)))
                            using (var irradianceUniformLayout =
                                factory.CreateResourceLayout(irradianceUniformLayoutDescription))
                            using (var irradianceResourceSet = factory.CreateResourceSet(
                        ...
                        kBRDF_LUT_Size, 1, 1, PixelFormat.R16_G16_Float, TextureUsage.Storage | TextureUsage.Sampled));
                    {
                        ...
                        var spbrdfUniformLayoutDescription = new ResourceLayoutDescription(
                            new ResourceLayoutElementDescription("LUT", ResourceKind.TextureReadWrite,
                                ShaderStages.Compute)
                        );

                        using (var shader =
                            factory.CreateFromSpirv(new ShaderDescription(ShaderStages.Compute, spbrdfShaderSrc, "main",
                                Debug)))
                        using (var spbrdfUniformLayout = factory.CreateResourceLayout(spbrdfUniformLayoutDescription))
                        using (var spbrdfResourceSet =
                            factory.CreateResourceSet(new ResourceSetDescription(spbrdfUniformLayout,
        ...
        protected override void CreateResources(ResourceFactory factory)
        {
            ...
            }
            {
                ...
                _modelMetalnessTexture =
                    metalnessTex.CreateDeviceTexture(GraphicsDevice, factory, TextureUsage.Sampled);
            }
            {
                var roughnessTex = LoadEmbeddedAsset<ProcessedTexture>("cerberus_R.binary");
                _modelRoughnessTexture =
            ...
            _shadingBuffer = factory.CreateBuffer(new BufferDescription((SizeOfLight * NumLights) + SizeOfVector4,
                BufferUsage.UniformBuffer));

            var pbrResourceLayout = factory.CreateResourceLayout(
                new ResourceLayoutDescription(
                    new ResourceLayoutElementDescription("TransformUniforms", ResourceKind.UniformBuffer,
                        ShaderStages.Vertex | ShaderStages.Fragment),
                    new ResourceLayoutElementDescription("ShadingUniforms", ResourceKind.UniformBuffer,
                        ShaderStages.Fragment),
                    new ResourceLayoutElementDescription("albedoTexture", ResourceKind.TextureReadOnly,
                        ShaderStages.Fragment),
                    new ResourceLayoutElementDescription("metalnessTexture", ResourceKind.TextureReadOnly,
                        ShaderStages.Fragment),
                    new ResourceLayoutElementDescription("roughnessTexture", ResourceKind.TextureReadOnly,
                        ShaderStages.Fragment),
                    new ResourceLayoutElementDescription("normalTexture", ResourceKind.TextureReadOnly,
                        ShaderStages.Fragment),
                    new ResourceLayoutElementDescription("irradianceTexture", ResourceKind.TextureReadOnly,
                        ShaderStages.Fragment),
                    new ResourceLayoutElementDescription("specularTexture", ResourceKind.TextureReadOnly,
                        ShaderStages.Fragment),
                    new ResourceLayoutElementDescription("specularBRDF_LUT", ResourceKind.TextureReadOnly,
                        ShaderStages.Fragment),
            ...
            var skyboxLayoutDescription = new ResourceLayoutDescription(
                new ResourceLayoutElementDescription("TransformUniforms", ResourceKind.UniformBuffer,
                    ShaderStages.Vertex),
                new ResourceLayoutElementDescription("envTexture", ResourceKind.TextureReadOnly, ShaderStages.Fragment),
                new ResourceLayoutElementDescription("textureSampler", ResourceKind.Sampler, ShaderStages.Fragment)
            );
            var skyboxResourceLayout = factory.CreateResourceLayout(ref skyboxLayoutDescription);
            ...
            _pbrResourceSet = factory.CreateResourceSet(
                new ResourceSetDescription(
                    pbrResourceLayout,
                    _matrixBuffer, _shadingBuffer,
                    _modelAlbedoTexture, _modelMetalnessTexture, _modelRoughnessTexture, _modelNormalTexture,
                    _modelIrradianceTexture, _environmentSpecularTexture, _modelSpecularBRDF_LUTTexture, textureSampler
                )
            );
            _skyboxResourceSet = factory.CreateResourceSet(
                new ResourceSetDescription(
                    skyboxResourceLayout,
                    _matrixBuffer,
                    _environmentSpecularTexture, textureSampler
                )
            );
            ...
            {
                new AnalyticalLight()
                    {Direction = new Vector4(-1.0f, 0.0f, 0.0f, 0.0f), Radiance = new Vector4(LightOff, 0f)},
                new AnalyticalLight()
                    {Direction = new Vector4(1.0f, 0.0f, 0.0f, 0.0f), Radiance = new Vector4(LightOff, 0f)},
                new AnalyticalLight()
                    {Direction = new Vector4(0.0f, -1.0f, 0.0f, 0.0f), Radiance = new Vector4(LightOff, 0f)},
            };
        ...
        protected override void Draw(float deltaSeconds)
        {
...

            {
                ...
                _commandList.SetGraphicsResourceSet(0, _skyboxResourceSet);
                ...
                _commandList.SetGraphicsResourceSet(0, _pbrResourceSet);
